...
Counter EQU 20h
SegDisplay EQU LATD ; Seven-segment display output on Port D
LimitH EQU 21h
LimitL EQU 22h
ZeroRange EQU 23h
DCounter EQU 24h
...
setup:
    ...
    MOVLW 0b11111111 ;
    MOVWF TRISB ;
    BANKSEL LATD ; Select LATD
    CLRF LATD ; Clear LATD
    BANKSEL TRISD ; Select TRISD
    CLRF TRISD ; Clear TRISD (set all as outputs)
    GOTO Loop
...
Loop:
    ; Check Button A
    BTFSC PORTB, 0 ; If Button A is pressed (((PORTB) and 0FFh), 0, a grounded)
    GOTO CheckBothButtons ; Go to IncCounter

    ; Check Button B
    BTFSC PORTB, 1 ; If Button B is pressed (((PORTB) and 0FFh), 1, a grounded)
    GOTO CheckBothButtonsB ; Go to DecCounter

    ; Check if both buttons are pressed


    RCALL UpdateDisplay ; Update the display with the current value
    RCALL Delay
    GOTO Loop ; Otherwise, continue looping
...
Delay:
    ...
    MOVLW 0x05 ; Outermost loop count = 3

    MOVWF OutermostLoopCount ; Store in register

OutermostLoop:

    MOVLW 0xFF ; Outer loop count = 255

    MOVWF OuterLoopCount ; Store in register

OuterLoop:

    MOVLW 0xFF ; Inner loop count = 255

    MOVWF InnerLoopCount ; Store in register

InnerLoop:

    DECFSZ InnerLoopCount, F ; Decrement inner loop counter and skip if zero

    GOTO InnerLoop ; Repeat inner loop


    DECFSZ OuterLoopCount, F ; Decrement outer loop counter and skip if zero

    GOTO OuterLoop ; Repeat outer loop


    DECFSZ OutermostLoopCount, F ; Decrement outermost loop counter and skip if zero

    GOTO OutermostLoop ; Repeat outermost loop
...
IncCounter:
    ...
    GOTO CheckLimits ; Check counter limits and update display
    Return
...
CheckLimits:
    ; Check if counter exceeds 0xF
    MOVF Counter, W
    CPFSGT LimitH ; If counter > 0xF
    CALL WrapAround ; Wrap around to 0x00
    GOTO UpdateDisplay ; Update the display

    ; Check if counter is zero
    CPFSEQ LimitL ; If counter == 0
    GOTO UpdateDisplay ; Update the display

    ; Counter is less than zero
    GOTO ResetCounterB ; Reset counter
...
ResetCounter:
    ...
    MOVWF Counter ; Move value to Counter
    GOTO UpdateDisplay ; Update the display
    RETURN
